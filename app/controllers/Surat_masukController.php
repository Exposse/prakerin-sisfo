...
class Surat_masukController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				surat_masuk.id LIKE ? OR 
				surat_masuk.tanggal_masuk LIKE ? OR 
				surat_masuk.tanggal_surat LIKE ? OR 
				surat_masuk.lampiran LIKE ? OR 
				surat_masuk.sifat_surat LIKE ? OR 
				surat_masuk.nomor_surat LIKE ? OR 
				surat_masuk.surat_dari LIKE ? OR 
				surat_masuk.perihal LIKE ? OR 
				surat_masuk.disampaikan_kpd LIKE ? OR 
				surat_masuk.disposisi LIKE ? OR 
				surat_masuk.catatan LIKE ? OR 
				surat_masuk.upload_file LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("surat_masuk.id", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("surat_masuk.id", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("tanggal_masuk","tanggal_surat","lampiran","sifat_surat","nomor_surat","surat_dari","perihal","upload_file");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","tanggal_masuk","tanggal_surat","lampiran","sifat_surat","nomor_surat","surat_dari","perihal","disampaikan_kpd","disposisi","catatan","upload_file");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
			);
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("surat_masuk.id", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("surat_masuk.id", $arr_rec_id, "in");
