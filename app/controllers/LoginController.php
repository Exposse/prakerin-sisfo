...
class LoginController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				login.id LIKE ? OR 
				login.upload_photo LIKE ? OR 
				login.photo LIKE ? OR 
				login.nama LIKE ? OR 
				login.user_name LIKE ? OR 
				login.pasword LIKE ? OR 
				login.email LIKE ? OR 
				login.role LIKE ?
			)";
			$search_params = array(
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("upload_photo","nama","user_name","pasword","email","role");
			...
			$password_text = $modeldata['pasword'];
			//update modeldata with the password hash
			$modeldata['pasword'] = $this->modeldata['pasword'] = password_hash($password_text , PASSWORD_DEFAULT);
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","upload_photo","nama","user_name","role");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'upload_photo' => 'required',
